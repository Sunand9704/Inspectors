...
async function seedPageWithSections() {
  try {
    ...
    if (!validation.isValid) {
      logger.error('âŒ Section validation failed. Details:');
    ...
    logger.error('âŒ Seeding failed:', error.message);
    process.exit(1);
  }
}
...
async function verifyData() {
  ...
  if (page) {
    ...
    logger.error('   âŒ Page not found');
  }
...
async function clearSeededData() {
  try {
    ...
    logger.error('âŒ Failed to clear data:', error.message);
    process.exit(1);
  }
}
